...
CONTAINS:
    ...
    - vis_allmat: function to visualize all matrices
...
def vis_allmat(matrices,dir):

    """
    imatrices: dict, all matrices
    
    RETURNS 
    
    """

    plot_path = os.path.join(dir, 'mat.png') 

    # Create a figure
    fig, axes = plt.subplots(1, 5, figsize=(15, 5))  # 1 row, 3 columns
    axes[0].imshow(matrices['uptake'], cmap='viridis')
    axes[0].set_title('uptake')
    axes[1].imshow(matrices['met'], cmap='plasma')
    axes[1].set_title('metabolism')
    axes[2].imshow(matrices['ess'], cmap='inferno')
    axes[2].set_title('essential')
    axes[3].imshow(matrices['spec_met'], cmap='winter')
    axes[3].set_title('specialization')
    axes[4].imshow(matrices['sign'], cmap='spring')
    axes[4].set_title('sign')
