...
def birth_death_biomass(N, g, biomass):
    ...
    epsilon=1e-13
    if (g<=epsilon).all():
        raise ValueError('No positive gr:community collapse')
    ...
    if not np.any(valid_mask):
        raise ValueError("No valid division times found. Community collapse.")
    ...
    if t_div > 100:
        raise ValueError(f"Division time too high: {t_div}. Community collapse.")
