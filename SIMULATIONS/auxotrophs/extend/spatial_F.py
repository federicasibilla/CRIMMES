...
def spatial_F(i):
    ...
    #----------------------------------------------------------------------------------------------------------------------
    # import data from old simulation
    pickle_file_path = Path(f'/work/FAC/FBM/DMF/smitri/nccr_microbiomes/federica/extend_aux/spatial_F_2_results/{i}/all_data.pkl')
    ...
    if pickle_file_path.exists():
        try:
            ...
            with open(pickle_file_path, 'rb') as file:
                ...
                # Skip the file if there's an error
    ...
    path = f'/work/FAC/FBM/DMF/smitri/nccr_microbiomes/federica/extend_aux/spatial_F_3'
    ...
    # spatial
    last_2_frames_N, _, current_R, current_N, g_rates, s_list, abundances, t_list, biomass  = update.simulate_3D_NBC(500000, fR, gr, R_space_ig, N0_space, biomass, param, mat_F)
    ...
    data = {
                                        ...
                                        'spec':spec_met,
                                        'mat': mat_F,
                                        'CR_R':df['CR_R_F'].iloc[i], 
                                        'CR_N':df['CR_N_F'].iloc[i], 
                                        'g0':df['g0_F'].iloc[i],
                                        'A':df['A_f'].iloc[i]
                                    }
    ...
    data['current_N']       = current_N
    data['g_rates']         = g_rates
    data['s_list']          = sh_sp[::200]+s_list[0::200]
    data['abundances']      = N_sp+abundances[0::200]
    data['t_list']          = t[::200]+t_list[0::200]
    data['biomass']         = biomass
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        ...
        print("Usage: python spatial_random_F.py")
